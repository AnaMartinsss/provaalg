...
numbers_input = input("Digite uma lista de números separados por vírgula: ")
#numbers_list = numbers_input.trim("")
squared_numbers = filter(lambda x: x * x, numbers_input)
even_squares = filter(lambda x: x % 2 == 0, squared_numbers)
total = reduce(lambda x,y: x + y, even_squares)
verificar_maior = sorted(even_squares) 

print(f"Quadrados: {squared_numbers}")
print(f"Quadrados pares: {even_squares}")
print(f"Soma: {total}")
print(f"Maior Quadrado Par: {even_squares[-1]}")
