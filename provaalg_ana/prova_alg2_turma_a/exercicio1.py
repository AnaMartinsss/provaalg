...
Objetivo:
Você recebeu um dicionário representando as vendas mensais de uma loja. 
Cada chave é o nome de um produto e o valor é uma lista de inteiros representando as vendas mensais.
Sua tarefa é:
  1. Criar uma função chamada `media_vendas` que calcule a média de vendas para cada produto.
  2. Retornar um novo dicionário contendo apenas os produtos cuja média de vendas seja maior 
  ou igual a um valor mínimo (parâmetro da função).
...
media_vendas = sum(vendas["Produto A", "Produto B","Produto C"]) / len(vendas["Produto A", "Produto B","Produto C"]) 
resultado = media_vendas(vendas, 150)
print(resultado)
